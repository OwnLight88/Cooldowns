...
function Cool.UI.Draw(key)
    ...
    if set.enabled then
        ...
        if container == nil then
            ...
            l:SetColor(1, 1, 1, 1)
            l:SetFont("$(BOLD_FONT)|$(KB_48)|thick-outline")
            l:SetVerticalAlignment(TOP)
            l:SetHorizontalAlignment(RIGHT)
            l:SetPixelRoundingEnabled(true)

						if set.stacks ~= nil then
								l:SetAnchor(BOTTOM, c, BOTTOM, 0, 5)
								local s = WM:CreateControl(key .. "_Stacks", c, CT_LABEL)
            		S:SetAnchor(TOPRIGHT, c, TOPRIGHT, -4, -4)
            		S:SetColor(1, 1, 1, 1)
            		S:SetFont("$(BOLD_FONT)|$(KB_40)|thick-outline")
            		S:SetVerticalAlignment(TOP)
            		S:SetHorizontalAlignment(RIGHT)
            		S:SetPixelRoundingEnabled(true)
						end
...
function Cool.UI.Update(setKey)
    ...
    local label = WM:GetControlByName(setKey .. "_Label")
		...
		local upTime = (set.timeOfProc + set.durationms - GetGameTimeMilliseconds()) / 1000

		local downTime = (set.timeOfProc + set.cooldownDurationMs - GetGameTimeMilliseconds()) / 1000
		...
		if (upTime <= 0) then
        		...
        		Cool.UI.PlaySound(Cool.preferences.sets[setKey].sounds.onReady)
						...
						if (downTime < 5) then
								label:SetText(string.format("%.1f", downTime))
						else
								label:SetText(string.format("%.0f", downTime))
						end
				end
				...
				if (upTime < 5) then
						label:SetText(string.format("%.1f", upTime))
	    	else
	        	label:SetText(string.format("%.0f", upTime))
	    	end
